...
export default function LeadsPage() {
  ...
  const handleApprove = async (id: number) => {
    setActionLoading(id);
    try { await approveBooking(id); await fetchBookings(); } catch (err) { console.error(err); }
          ...
          {filtered.map(booking => {
            ...
            return (
              <div key={booking.id} className="bg-card rounded-xl border border-border shadow-sm overflow-hidden">
                <div className="p-5 cursor-pointer hover:bg-muted/30 transition-colors" onClick={() => setExpandedId(isExpanded ? null : booking.id)}>
                ...
                {isExpanded && (
                  <div className="border-t border-border p-5 bg-muted/20">
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4">
                      ...
                      {parsed.poolAccess && <div><span className="text-xs font-medium text-muted-foreground uppercase tracking-wide">Pool Access</span><p className="text-foreground mt-0.5 capitalize">{parsed.poolAccess}</p></div>}
                      {parsed.preferredDays && parsed.preferredDays.length > 0 && <div><span className="text-xs font-medium text-muted-foreground uppercase tracking-wide">Preferred Days</span><p className="text-foreground mt-0.5">{Array.isArray(parsed.preferredDays) ? parsed.preferredDays.join(", ") : parsed.preferredDays}</p></div>}
                      {parsed.experience && <div><span className="text-xs font-medium text-muted-foreground uppercase tracking-wide">Experience</span><p className="text-foreground mt-0.5">{parsed.experience}</p></div>}
                      {booking.preferredDate && <div><span className="text-xs font-medium text-muted-foreground uppercase tracking-wide">Preferred Date</span><p className="text-foreground mt-0.5">{booking.preferredDate}</p></div>}
                      {booking.preferredTime && <div><span className="text-xs font-medium text-muted-foreground uppercase tracking-wide">Preferred Time</span><p className="text-foreground mt-0.5">{booking.preferredTime}</p></div>}
                    </div>
                    ...
                    <div className="flex flex-wrap gap-2 pt-2">
                      {booking.status === "pending" && (
                        <>
                          <button onClick={() => handleApprove(booking.id)} disabled={actionLoading === booking.id}
                          ...
                          {rejectNoteId === booking.id ? (
                            <div className="flex items-center gap-2 flex-1">
                              <input type="text" value={rejectNote} onChange={e => setRejectNote(e.target.value)} placeholder="Rejection reason (optional)"
                                className="flex-1 px-3 py-2 rounded-lg border border-input bg-background text-sm text-foreground focus:outline-none focus:ring-2 focus:ring-ring" autoFocus />
                              <button onClick={() => handleReject(booking.id)} disabled={actionLoading === booking.id}
                                className="px-4 py-2 rounded-lg bg-destructive text-destructive-foreground text-sm font-medium hover:opacity-90 disabled:opacity-50">Confirm</button>
                              <button onClick={() => { setRejectNoteId(null); setRejectNote(""); }}
                                className="px-3 py-2 rounded-lg bg-secondary text-secondary-foreground text-sm">Cancel</button>
                            </div>
                          ) : (
                            <button onClick={() => setRejectNoteId(booking.id)}
                              className="px-4 py-2 rounded-lg bg-destructive/10 text-destructive text-sm font-medium hover:bg-destructive/20 transition-colors">Reject</button>
                          )}
                        </>
                      )}
                      {booking.status === "approved" && (
                        <button onClick={() => copyCalendlyLink(booking.id)}
