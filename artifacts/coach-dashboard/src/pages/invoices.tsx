...
export default function InvoicesPage() {
  ...
  const load = async () => {
    try {
      const [inv, c, s] = await Promise.all([getInvoices(), getClients(), getSessions()]);
      setInvoices(inv);
      setClients(c);
      setSessions(s);
    } catch (err) {
  ...
  const sessionLabel = (id?: number) => {
    ...
    const date = s.scheduledAt ? new Date(s.scheduledAt).toLocaleDateString() : "Unscheduled";
    return `${date}${s.service ? ` · ${s.service}` : ""}`;
  };

  const handleMarkPaid = async (id: number, method?: string) => {
    setActionLoading(id);
    try {
      await updateInvoice(id, { status: "paid", paymentMethod: method });
      await load();
    } catch (err) {
      console.error(err);
    }
    ...
    <div>
      <div className="mb-6 flex items-start justify-between gap-4">
        <div>
          ...
          <p className="text-muted-foreground mt-1">Track payments — cash, Venmo, Zelle</p>
        </div>
        <button
      ...
      {invoices.length > 0 && (
        <div className="grid grid-cols-2 gap-4 mb-6">
          <div className="bg-card rounded-xl border border-border p-4 shadow-sm">
            <p className="text-xs text-muted-foreground font-medium uppercase tracking-wide mb-1">Outstanding</p>
            <p className="text-2xl font-bold text-foreground">{formatDollars(totalOutstanding)}</p>
          </div>
          <div className="bg-card rounded-xl border border-border p-4 shadow-sm">
            <p className="text-xs text-muted-foreground font-medium uppercase tracking-wide mb-1">Total Paid</p>
            <p className="text-2xl font-bold text-green-600">{formatDollars(totalPaid)}</p>
          </div>
        </div>
      )}
      ...
      {showCreate && (
        <div className="bg-card border border-border rounded-xl p-5 mb-6 shadow-sm">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            ...
            <div className="space-y-1.5">
              <label className="text-xs font-medium text-muted-foreground uppercase tracking-wide">Linked Session</label>
              <select
            ...
            <div className="space-y-1.5 md:col-span-2">
              ...
              <input
                ...
                placeholder="e.g. 3-session package discount applied"
                className="w-full h-10 rounded-lg border border-input bg-background px-3 text-sm text-foreground focus:outline-none focus:ring-2 focus:ring-ring"
              />
            </div>
          </div>
          <div className="flex gap-2 mt-4">
            <button
              ...
              disabled={saving || !form.amountDollars}
      ...
      {filtered.length === 0 ? (
        <div className="text-center py-12 text-muted-foreground">
          <svg className="w-12 h-12 mx-auto mb-3 opacity-40" fill="none" viewBox="0 0 24 24" stroke="currentColor">
            <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1.5} d="M9 7h6m0 10v-3m-3 3h.01M9 17h.01M9 11h.01M12 11h.01M15 11h.01M12 7h.01M12 14h.01M15 14h.01M9 14h.01M5 3h14a2 2 0 012 2v14a2 2 0 01-2 2H5a2 2 0 01-2-2V5a2 2 0 012-2z" />
          </svg>
              ...
              <div className="flex items-start justify-between gap-4">
                <div className="flex-1 min-w-0">
                  <div className="flex items-center gap-3 mb-1 flex-wrap">
                    ...
                    <span className={`px-2.5 py-0.5 rounded-full text-xs font-medium border ${STATUS_COLORS[invoice.status] || "bg-gray-100 text-gray-800 border-gray-200"}`}>
                  ...
                  {invoice.notes && (
                    <p className="text-sm text-muted-foreground mt-1.5 italic">{invoice.notes}</p>
                  )}
                </div>
                <div className="flex gap-2 flex-shrink-0 flex-wrap justify-end">
                  {invoice.status === "draft" && (
                    <button
                      ...
                      className="px-3 py-1.5 rounded-lg bg-yellow-100 text-yellow-800 text-xs font-medium hover:bg-yellow-200 disabled:opacity-50"
                    >
                      {actionLoading === invoice.id ? "..." : "Mark Sent"}
                    </button>
                  )}
                  {invoice.status !== "paid" && (
                    <button
